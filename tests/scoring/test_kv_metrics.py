...
def test_parse_missing_series_is_none() -> None:
    ...
    assert parse_kv_cache_perc(text) is None
    assert parse_kv_cache_perc("") is None


def test_fetch_kv_cache_perc_ok(remock_kv: Callable[[httpx.Response], None]) -> None:
    remock_kv(httpx.Response(200, text="vllm:kv_cache_usage_perc 0.5\n"))
    with httpx.Client() as client:
        assert fetch_kv_cache_perc(client, "http://127.0.0.1:8000/metrics") == 50.0


def test_fetch_kv_cache_perc_404_is_none(
    remock_kv: Callable[[httpx.Response], None],
) -> None:
    remock_kv(httpx.Response(404, text="no"))
    with httpx.Client() as client:
